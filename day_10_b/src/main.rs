...
fn is_path(map: &Map, pos: IVec2, next_level: u8, acc_res: &mut i32) {
    if next_level == 10 {
        *acc_res += 1;
        return;
    }
    ...
    for dir in [ivec2(-1, 0), ivec2(0, -1), ivec2(1, 0), ivec2(0, 1)] {
        if map.at(pos + dir) == next_level {
            is_path(map, pos + dir, next_level + 1, acc_res)
        }
    }
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for y in 0..map.h {
        for x in 0..map.w {
            if map.at(ivec2(x, y)) == 0 {
                is_path(&map, ivec2(x, y), 1, &mut res);
            }
        }
    }
